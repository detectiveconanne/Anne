import java.text.ParseException; //If the input doesn't match the required format, a ParseException is thrown. This is caught and handled to notify the user of invalid input.
import java.text.SimpleDateFormat; //Converts user-inputted strings into Date objects for validation (e.g., checking if the date is in the future).
import java.util.ArrayList; //Each Appointment object is added to the list when an appointment is booked.
import java.util.Date; // it ensures that an entered date is in the future by comparing it to the current date.
